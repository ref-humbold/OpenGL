                            ...
                            GLfloat radius, vec3 pos)
{
    ...
    for(int i = 0; i < 8; ++i)
        glDrawElements(GL_TRIANGLE_STRIP, 42, GL_UNSIGNED_SHORT,
                       (void *)((42 * i + 22) * sizeof(unsigned short)));
...
void GameBubble::move(GLfloat delta, GLfloat side)
{
    ...
    auto isOut = [=](std::tuple<GLfloat, vec3, vec3, int> b) { return std::get<1>(b)[1] >= side; };
...
void GameBubble::countNormals()
{
    ...
    {
        vec3 n =
            normalize(vec3(nbDataBubble[4 * i], nbDataBubble[4 * i + 1], nbDataBubble[4 * i + 2]));
